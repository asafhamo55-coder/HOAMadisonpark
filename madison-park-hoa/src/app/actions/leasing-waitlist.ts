...
export async function getLeasingData(): Promise<{
  ...
  const [propertiesRes, waitlistRes] = await Promise.all([
    ctx.supabase.from("properties").select("id, address, occupancy_type"),
    ctx.supabase
...
export async function addToWaitlist(formData: FormData) {
  ...
  const { error } = await ctx.supabase.from("leasing_waitlist").insert({
  ...
  revalidatePath(tenantPath(ctx.tenantSlug))
  revalidatePath(tenantPath(ctx.tenantSlug, "leasing"))
  return { error: null }
}
...
export async function approveFromWaitlist(id: string) {
  ...
  revalidatePath(tenantPath(ctx.tenantSlug))
  revalidatePath(tenantPath(ctx.tenantSlug, "leasing"))
  revalidatePath(tenantPath(ctx.tenantSlug, "properties"))
  return { error: null }
}
